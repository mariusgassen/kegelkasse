...
| 33 | **Kamera-Wurf-Erkennung (Echtzeit)** | ⬜    | Vision-Sidecar (Python/OpenCV, ~30fps Loop): Webcam/IP-Kamera erkennt LED-Lämpchen der analogen Kegelbahn in Echtzeit → Zustandsänderung der 9 Pin-Lämpchen per Helligkeitsschwelle + Debounce → sofortiger POST an Backend → WebSocket-Broadcast (`/ws/evening/{id}`) an alle verbundenen Clients (<1s Latenz); Kalibrierungs-Tool (ROI per Drag-and-Drop für 9 Kegel); Spieler-Queue-Sync; Frontend: Kamera-Modus-Toggle, Live-Overlay (aktueller Spieler, Echtzeit-Pins), Bestätigungs-Sheet mit 5s Auto-Confirm + manuellem Override; kompatibel mit Tablet-Schnellerfassung (#29); extra Docker-Container optional; Phasen: 1) Sidecar+Kalibrierung+WebSocket-Backend, 2) App-Echtzeit-UI, 3) ML-Fallback+Genauigkeits-Log |
