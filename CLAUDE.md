...
| 44 | **Mitglied aus Verein entfernen**  | ✅      | `DELETE /regular-members/{mid}` konvertiert reguläres Mitglied zu Gast (`is_guest=True`, Login gesperrt, Pins gelöscht) statt vollständiger Deaktivierung — Mitglied bleibt im Gäste-Roster sichtbar und kann weiter an Abenden teilnehmen; Gast-Löschung deaktiviert vollständig (`is_active=False`); `PATCH /regular-members/{mid}/reactivate` setzt `is_guest=False` + reaktiviert Account; ⬆️-„Zu Mitglied machen"-Knopf im Gäste-Roster (Admin) — Wortwahl/Icon bewusst neutral gegenüber „Reaktivieren", da auch Gäste promoviert werden, die nie Mitglied waren; Bestätigungs-Sheet bei Entfernen; **Eintrittsbeitrag bei Beförderung**: Bestätigungs-Sheet schlägt einen anteiligen Eintrittsbeitrag vor = Kassenstand (Summe aller Mitgliedssalden inkl. offener Schulden) ÷ Anzahl bestehender Mitglieder; Admin kann den Betrag anpassen oder löschen; bei Bestätigung wird er als Schuld (negativer `MemberPayment`-Eintrag, Notiz „Eintrittsbeitrag") auf dem neuen Mitgliedskonto gebucht — rein frontend-seitig orchestriert über die bestehenden `reactivateRegularMember`- und `createMemberPayment`-Endpoints, kein neuer Backend-Code; pytest: 9 Tests (Entfernen/Reaktivieren); Vitest: 3 neue Tests (Sheet-Vorbefüllung, Bestätigung mit/ohne Beitrag). |
