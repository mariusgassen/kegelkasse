...
| 3  | **Kasse**                          | ✅      | Ranking, Spiele/Getränke, Text-Export (Share/Copy); **Saldo-Verlauf-Graph** (📈 Verlauf) in der Übersicht: SVG-Step-Chart mit Kasse/Mitglied-Umschalter (`ModeToggle`) — Kasse = Vereins-Cashflow (Zahlungen+Ausgaben), Mitglied = Pille-Liste aller Mitglieder (eigenes Konto zuerst, Ich-Badge); Monat/Jahr/Alle-Ansichten mit durchgehender kumulativer Basislinie (Blättern setzt Kurve nie auf Null zurück), bei „Alle" horizontal scrollbar mit fixierter Y-Achse; zwei parallele Linien „Tatsächlich" (reale Buchungen) und „Inkl. Schulden"/„Inkl. Strafen" (virtueller Saldo inkl. offener Schulden zum jeweiligen Zeitpunkt via `GET /club/treasury-debt-timeline`, bzw. inkl. verbuchter Strafen via `GET /club/member-penalties/{mid}`); klickbare Punkte zeigen Datum/Art/Betrag der zugrunde liegenden Buchung sowie den daraus resultierenden kumulativen Saldo beider Linien, Schulden-Stützpunkte sind nicht antippbar; „Tatsächlich"-Linie nutzt `var(--kce-cream)` statt eines hartkodierten Hex-Werts, damit sie unter jedem per-Club-Theme (Feature #31) sichtbar bleibt; „Inkl. Schulden/Strafen"-Linie (Dot-Highlight + Legende) nutzt `var(--kce-primary)` statt eines nicht existenten `var(--kce-amber)` (nur `--color-kce-amber` existiert als Tailwind-Theme-Token, keine literale `:root`-Variable — der Stroke resolvte zu `none` und war unsichtbar); `lib/balanceHistory.ts` (pure logic, 24 Vitest-Tests) |
