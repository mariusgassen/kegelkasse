...
| 29 | **Tablet Schnellerfassung**        | ✅      | Vollbild-Overlay (⚡) für Landscape-Modus; 3-Spalten-Layout: Spieler (Auswahl) \| Strafen + Getränke \| **Übersicht pro Spieler** (Strafen-€-Summe inkl. Gäste-Cap, Spiel-Score live aus letztem Wurf, Getränke-Counts, Plausibilitäts-Check + Gesamtsumme im Footer); iOS safe-area-insets (Notch, Home-Indicator, gerundete Ecken); kein Scrollen erforderlich; letzte Einträge unten; 0€-Strafen ausgeblendet; Strafen-Buttons vergrößert. **Getränkerunden direkt im Strafen-Panel** als 🍺/🥃-Buttons — gleiche Spieler-Mehrfachauswahl wie Strafen, kein Bottom-Sheet und kein versteckter Header-CTA mehr (Runde wird sofort für die ausgewählten Spieler gebucht, Auswahl danach zurückgesetzt). Übersicht-Spalte nach Strafen-€ absteigend sortiert mit FLIP-Reorder-Animation. **Footer-Σ/Ø korrigiert:** Σ Strafen entspricht jetzt exakt dem Kassen-Total (ProtocolPage `euroTotal`: gedeckelte anwesende Spieler **+ Abwesenheits-/Nachtrags-Strafen zum vollen Betrag**, vorher fehlten Letztere); Ø Strafen nutzt die **tatsächlichen, ungedeckelten** Strafen pro anwesendem Spieler, damit der Gäste-Deckel (reines Kassen-Artefakt) den Plausibilitäts-Durchschnitt nicht verfälscht (die Einzelzeile zeigt weiterhin den gedeckelten Betrag). |
