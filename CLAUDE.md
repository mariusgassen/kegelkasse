...
| 43 | **Strafen × Getränke-Korrelation** | ✅      | Jahresrückblick-Sektion mit 3 Tabs: **Pro Abend** (Scatter € vs Runden + Trendlinie + Pearson r + Plain-Language-Slope "Pro +€1 ≈ +X Getränke" + Quartil-Vergleich Top 25 % vs Bottom 25 %), **Pro Mitglied** (Per-(Mitglied×Abend)-Scatter — ein Punkt pro Mitglied & Abend, farbcodiert, Pill-Legend zum Fokussieren auf ein Mitglied, dann Trendlinie + persönlicher r), **Korrelations-Stärke** (Balken pro Mitglied nach \|r\|). Eigenes Panel in der Abend-Detail-Sektion: Pill-Picker (Mitglied + Bin-Größe 5/15/30 min), Dual-Axis-Line (kumuliert Strafen + Getränke), Δ-Bar-Chart pro Bin, Pearson r der Veränderungen. GET /stats/correlation/{year} liefert pro Mitglied evening_points; GET /stats/correlation/evening/{evening_id}?bin_minutes. Bier+Schnaps kombiniert. Pearson-Helper (stdlib, n≥3, Varianz-Guard). pytest: 18 Tests; Vitest: lib/stats (pearson + linearRegression + interpretR), API-Client, StatsPage-Tab-Render. |
