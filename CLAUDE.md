...
| 33 | **Kamera-Wurf-Erkennung (Echtzeit)** | 🚧    | Phase 1+2 implementiert: CameraCapturePage (fullscreen overlay aus GamesPage, Admin-only); cameraEngine.ts (7-Segment-Erkennung + Pin-Glob-Detektion, grüner Kanal, deterministisch); Kalibrierungs-UI (Drag-and-Drop ROI für 4 Bereiche per SVG/PointerEvents, Digit-Anzahl, Helligkeitsschwelle, localStorage); Erkennungs-Loop (RAF, 10+ fps); 5s Auto-Confirm-Overlay mit Glob-Validierung (Globenzahl vs. Display-Mitte); Wurf-Protokoll; Spiel-Beenden direkt aus Camera-Ansicht via api.finishGame(); Phase 2: game_throw_log-Tabelle (Migration 037, Upsert per throw_num), POST /evening/{eid}/games/{gid}/throws (Admin, triggers SSE), clear on startGame; serialize_evening enthält throws pro Spiel; GamesPage zeigt Live-Wurf-Indikator (📷 Wurf # · Kegel · Σ) für laufende Spiele; alle Clients sehen Echtzeit-Updates via SSE <1s; Phasen: 1) ✅ Kalibrierung + Segment/Glob-Erkennung, 2) ✅ Backend + Echtzeit-UI, 3) Genauigkeits-Log. Vollmer-Anlage: client-seitig im Browser (kein Sidecar), Kamera am Spieler-Ende auf Anzeigetafel; vier Erkennungs-Kanäle: (1) 9 Pin-Globen (Rautenformation, Helligkeitsschwelle: leuchtet=gefallen, aus=steht), (2) Display Links = Wurf-Nummer (Event-Trigger: Zahl ändert sich → neuer Wurf), (3) Display Mitte = Kegel dieses Wurfs (primäre Score-Quelle), (4) Display Rechts = kumulierter Gesamtstand (Konsistenzcheck); alle Displays: grüne 7-Segment-Digits auf schwarz → maximaler Kontrast, deterministisches Segment-Mapping (7 ROIs/Stelle, kein OCR/ML); Ablauf: Wurf-Nr.-Änderung erkannt → Mitte lesen → gegen Pin-Globen validieren → bei Match Auto-Confirm, bei Abweichung manuelle Bestätigung; API-POST → WebSocket-Broadcast an alle Evening-Clients (<1s Latenz); Kalibrierungsansicht (Live-Vorschau, Drag-and-Drop ROI für alle 4 Bereiche, in localStorage); dedizierte Admin-Ansicht "Kamera-Erfassung"; 5s Auto-Confirm-Overlay + Override; kompatibel mit #29; Phasen: 1) ✅ Kalibrierung + Segment/Glob-Erkennung, 2) WebSocket-Backend + Echtzeit-UI, 3) Genauigkeits-Log | client-seitig im Browser (kein Sidecar), Kamera am Spieler-Ende auf Anzeigetafel; vier Erkennungs-Kanäle: (1) 9 Pin-Globen (Rautenformation, Helligkeitsschwelle: leuchtet=gefallen, aus=steht), (2) Display Links = Wurf-Nummer (Event-Trigger: Zahl ändert sich → neuer Wurf), (3) Display Mitte = Kegel dieses Wurfs (primäre Score-Quelle), (4) Display Rechts = kumulierter Gesamtstand (Konsistenzcheck); alle Displays: grüne 7-Segment-Digits auf schwarz → maximaler Kontrast, deterministisches Segment-Mapping (7 ROIs/Stelle, kein OCR/ML); Ablauf: Wurf-Nr.-Änderung erkannt → Mitte lesen → gegen Pin-Globen validieren → bei Match Auto-Confirm, bei Abweichung manuelle Bestätigung; API-POST → WebSocket-Broadcast an alle Evening-Clients (<1s Latenz); Kalibrierungsansicht (Live-Vorschau, Drag-and-Drop ROI für alle 4 Bereiche, in localStorage); dedizierte Admin-Ansicht "Kamera-Erfassung"; 5s Auto-Confirm-Overlay + Override; kompatibel mit #29; Phasen: 1) Kalibrierung + Segment/Glob-Erkennung, 2) WebSocket-Backend + Echtzeit-UI, 3) Genauigkeits-Log |
