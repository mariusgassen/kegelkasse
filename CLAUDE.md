...
| 43 | **Strafen × Getränke-Korrelation** | ✅      | Neue Korrelations-Sektion im Jahresrückblick mit 4 Tabs: **Pro Abend** (Scatter € vs Runden + Trendlinie + Pearson r), **Pro Mitglied** (Scatter Gesamt-€ vs Gesamt-Runden, Punktgröße = Abende), **Korrelations-Stärke** (Balken pro Mitglied nach \|r\|), **Verlauf an einem Abend** (Dual-Axis-Line: kumulierte Strafen + Getränke pro 5/15/30-Min-Bin, Pearson r der Veränderungen). GET /stats/correlation/{year} + GET /stats/correlation/evening/{evening_id}?bin_minutes. Bier+Schnaps kombiniert. Pearson-Helper (stdlib, n≥3, Varianz-Guard). pytest: 17 Tests; Vitest: lib/stats (pearson + linearRegression + interpretR), API-Client, StatsPage-Tab-Render. |
