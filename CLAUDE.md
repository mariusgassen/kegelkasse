...
| 44 | **Mitglied aus Verein entfernen**  | ✅      | `DELETE /regular-members/{mid}` deaktiviert jetzt auch den verknüpften User-Account (kein Login mehr); `PATCH /regular-members/{mid}/reactivate` reaktiviert Mitglied + Account; `GET /regular-members?include_inactive=true` für Admins; Ehemalige-Sektion im Roster mit Reaktivierungsknopf; Bestätigungs-Sheet bei Entfernen; pytest: 8 Tests. |
| 45 | **Kassen-Auszahlung**              | ✅      | Bulk-Auszahlung aus der Vereinskasse an Mitglieder: `POST /club/treasury-payout` erzeugt negative MemberPayment-Einträge (Betrag pro Mitglied variabel); 💸-Button in der Kasse-Übersicht (Admin); Auszahlungs-Sheet mit aktuellen Salden und individuellen Betragsfeldern; pytest: 6 Tests. |
