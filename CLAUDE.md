...
| 33 | **Kamera-Wurf-Erkennung**          | ⬜      | Vision-Sidecar (Python/OpenCV): Webcam/IP-Kamera erkennt LED-Lämpchen der analogen Kegelbahn → leitet Pins/Punkte automatisch ab → POST an `/api/v1/vision/throw`; einmaliges Kalibrierungs-Tool (ROI per Drag-and-Drop für 9 Kegel-Positionen); Spieler-Queue-Sync mit App; Frontend: Kamera-Modus-Toggle, Live-Status-Overlay (aktueller Spieler, letzter Wurf), Bestätigungs-Sheet mit 5s Auto-Confirm und manueller Korrektur; extra Docker-Container optional; Phasen: 1) Sidecar+Kalibrierung, 2) API+App-UI, 3) ML-Fallback+Genauigkeits-Log |
