...
| 33 | **Kamera-Wurf-Erkennung (Echtzeit)** | ⬜    | Client-seitig im Browser (kein Sidecar): `getUserMedia()` → Canvas-Frame-Analyse in JS; drei Erkennungs-Kanäle parallel: (1) 9 Pin-LEDs per Helligkeitsschwelle + ROI, (2) 7-Segment-Display Wurf 1, (3) 7-Segment-Display Wurf 2; Segment-Mapping: 7 ROIs/Stelle, binär on/off → Ziffer deterministisch, kein OCR/ML; Kreuzvalidierung: Pin-Zählung vs. Display-Wert → bei Übereinstimmung Auto-Confirm, bei Abweichung manuelle Bestätigung; Debounce auf Zustandswechsel → sofortiger API-POST → WebSocket-Broadcast an alle Evening-Clients (<1s Latenz); Kalibrierungsansicht im Browser (Live-Vorschau, Drag-and-Drop ROI für Pins + beide Segmentdisplays, gespeichert in localStorage); dedizierte Admin-Ansicht "Kamera-Erfassung"; Bestätigungs-Overlay mit 5s Auto-Confirm + Override; kompatibel mit Tablet-Schnellerfassung (#29); Phasen: 1) Kalibrierung + Erkennung aller 3 Kanäle, 2) Kreuzvalidierung + WebSocket-Backend + Echtzeit-UI, 3) Genauigkeits-Log |
