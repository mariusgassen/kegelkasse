...
| 33 | **Kamera-Wurf-Erkennung (Echtzeit)** | ⬜    | Client-seitig im Browser (kein Sidecar): `getUserMedia()` → Canvas-Frame-Analyse in JS; zwei Erkennungs-Kanäle: (1) 9 Pin-LEDs per Helligkeitsschwelle + ROI, (2) 7-Segment-Display per Segment-Mapping (7 ROIs/Stelle, binär on/off → Ziffer deterministisch, kein OCR/ML nötig); Debounce auf Zustandswechsel → sofortiger API-POST → WebSocket-Broadcast an alle Evening-Clients (<1s Latenz); Kalibrierungsansicht im Browser (Live-Vorschau, Drag-and-Drop ROI für Pins + Segmente, gespeichert in localStorage); dedizierte Admin-Ansicht "Kamera-Erfassung" auf Gerät an der Bahn; Bestätigungs-Overlay mit 5s Auto-Confirm + Override; kompatibel mit Tablet-Schnellerfassung (#29); Phasen: 1) Kalibrierung + Segment/Pin-Erkennung im Browser, 2) WebSocket-Backend + App-Echtzeit-UI, 3) Genauigkeits-Log + Fallback-Manuell |
