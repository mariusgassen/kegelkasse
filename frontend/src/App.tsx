...
export default function App() {
  ...
  useEffect(() => {
    if (user?.preferred_locale) {
      const { setLocale } = require('./i18n').useI18n.getState()
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%', maxWidth:640, margin:'0 auto' }}>
        ...
        <div className="flex items-center gap-2.5 px-3 pb-2 pt-1">
          ...
          <div className="flex gap-1 flex-shrink-0">
            {(['de','en'] as const).map(l => {
              const { locale: cur, setLocale } = require('./i18n').useI18n.getState()
              return (
                <button key={l} onClick={() => { setLocale(l); api.updateLocale(l).catch(()=>{}) }}
                  className={`text-[9px] font-extrabold px-1.5 py-0.5 rounded ${cur===l?'bg-kce-amber text-kce-bg':'text-kce-muted'}`}>
                  {l.toUpperCase()}
                </button>
              )
            })}
      ...
      <main style={{ flex:1, overflow:'hidden', position:'relative' }}>
        {([
          ...
          ['stats',     <StatsPage />],
          ['club',      <ClubAdminPage />],
        ] as [PageId, React.ReactNode][]).map(([id, el]) => (
