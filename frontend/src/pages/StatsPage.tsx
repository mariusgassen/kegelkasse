      ...
      {!evening || !eveningStats ? (
        ...
        <>
          <div className="grid grid-cols-2 gap-2 mb-4">
            ...
            <StatBox value={String(eveningStats.beerRounds)} label={t('treasury.drinks').replace('🍺 ','')} />
      ...
      {!yearStats ? (
        ...
        <>
          ...
          {yearStats.players.slice(0, 5).map((p: any, i: number) => {
            const colors = [{bg:'#e8a020',tc:'#0a0600'},{bg:'#909090',tc:'#fff'},{bg:'#b07030',tc:'#fff'}]
            ...
            {evening.players.map(p => {
              ...
              return (
                <div key={p.id} className="kce-card p-3">
                  <div className="w-10 h-10 rounded-full flex items-center justify-center font-display font-bold text-kce-bg text-sm mb-2"
                  ...
                  <div className="flex justify-around text-center">
                    <div><div className="text-kce-amber font-bold text-sm">{wins}</div><div className="text-[9px] text-kce-muted">Siege</div></div>
                    <div><div className="text-red-400 font-bold text-sm">{fe(pTotal)}</div><div className="text-[9px] text-kce-muted">Strafen</div></div>
                    <div><div className="text-kce-amber font-bold text-sm">🍺{beerC}</div><div className="text-[9px] text-kce-muted">Bier</div></div>
                  </div>
                </div>
              )
            })}
