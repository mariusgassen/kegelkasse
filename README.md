...
- **Balance-history graph**: interactive SVG chart on the Overview tab showing the running balance over time as a step line; toggle between **Kasse** (overall club cash) and **Mitglied** (any individual member's personal balance, with an "Ich"-badged member picker); **Month / Year / All** views with continuous cumulative paging (never resets to zero), a y-axis with € labels on every view (previously only on "All"), and a horizontally scrollable "All" timeline with a fixed y-axis; the Month/Year x-axis clusters points onto discrete evenly-spaced buckets — one per active calendar day ("evening") in Month view, one per calendar month in Year view — instead of continuous-time spacing, so a handful of bookings in an otherwise quiet period aren't squeezed into one corner; two parallel lines — for **Kasse**: "Tatsächlich" (real bookings) vs. "Inkl. Schulden" (incl. outstanding debt); for **Mitglied**: "Eingezahlt" (payments made) vs. "Saldo" (balance after penalties) — scope-specific wording replacing the previous "Tatsächlich"/"Inkl. Strafen" pair, which read oddly without the Kasse context; clicking a point reveals the underlying booking/penalty (date, kind, amount). **Clustered markers**: points sharing the same x-axis bucket (and drawn on the same actual/overlay curve) collapse into a single clickable marker (with a "×N" badge) instead of stacked, mutually-hiding circles where only the last-drawn one could be clicked — clicking a cluster lists every underlying booking/penalty in the details view. Pure `clusterPoints()` in `lib/balanceHistory.ts`, tested.
...
- **Filter balance by players (🔍 Nach Spielern filtern)**: a collapsible member picker right below the personal "Mein Konto" card — and above the Kassenstand hero — on the Overview tab scopes the page **globally** to a selection of members, positioned so the control precedes everything it affects rather than following it. Two modes — **Exclude selected** writes off the selected members' outstanding debt (already-paid stays, open penalties no longer count — useful e.g. for players who might soon leave the club) using the realistic worst-case clearing rule instead of the optimistic "everyone eventually pays" baseline; **Only selected** restricts everything to just the selected members, a plain subset filter. The filter now drives the Kassenstand hero and its money-flow breakdown rows (paid-in/outstanding/cash-on-hand/projected), the "Offen & Guthaben" tiles/lists, and — in 🏛️ Kasse scope — the "Tatsächlich" (actual cash) line of the 📈 Verlauf history graph; club-wide expenses and the debt-timeline overlay (not attributable to individual members) stay unfiltered, guests are never part of the selectable filter, and the Konten tab remains the whole-club view. Pure logic in `lib/treasurySummary.ts` (`writeOffOutstandingDebt`)
