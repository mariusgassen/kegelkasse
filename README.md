...
- **Audio call-outs**: a club-wide toggle (Verein → Einstellungen → Audio-Ansagen, default on) for an optional synthesized sound picked per penalty type from a preview-able picker next to its icon — e.g. assign the buzzer to a "0 pins" penalty type instead of a separate hardcoded trigger. 13 presets: referee whistle, buzzer, air horn, bell, cash register, coin drop, sad trombone, crowd groan, applause, drum hit, record scratch, boing, laser. All sounds are generated with the Web Audio API (no audio files) and share the same audio context as the celebration chimes. Gated by both the club switch and the personal 🎉 effects toggle. The synthesis goes well beyond plain oscillators — inharmonic partial stacks (bell, coin), FM (cash register, laser), band-passed noise off a shared buffer (applause, drum, breath), `playbackRate` automation (record scratch), parallel band-pass formants (crowd groan), wave-shaper grit (buzzer, air horn) and LFOs (whistle trill, trombone vibrato) — and every preset runs through one shared compressor + short convolution room so the catalog sits at a comparable loudness. `frontend/scripts/audit-sounds.mjs` renders each preset through an `OfflineAudioContext` in headless Chromium and checks level, duration, attack and spectral balance
