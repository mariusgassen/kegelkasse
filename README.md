...
- **History detail shows the same chronological timeline as the live protocol** — tapping a closed evening's card expands its summary stats plus a "🕒 Verlauf" event ticker (penalties and drink rounds in order, newest first, same shared `EventTicker` component as the live cockpit) instead of only aggregated per-player totals and a drink count; no more reopening an evening just to see what happened in what order
