...
- **Easter egg** — tapping the club logo/title in the header 5× quickly opens a hidden mini 9-pin bowling game: aim with a sweeping guide (tap to lock), set power with a sweeping meter (tap to throw), then watch the ball scatter the diamond rack. Three balls per game, clearing all nine re-racks for a bonus. Finished games are submitted to a **club-wide top-10 leaderboard** (backend-persisted, attributed by Kegelname), and once a player has found the Easter egg the leaderboard is revealed in their **profile** (Meine Saison tab). A local best is kept as an offline fallback
