...
- **Filter balance by players (🔍 Nach Spielern filtern)**: a collapsible member picker above the "Offen & Guthaben" tiles on the Overview tab scopes those totals and lists to a selection of members, with two modes — **Exclude selected** writes off the selected members' outstanding debt (already-paid stays, open penalties no longer count — useful e.g. for players who might soon leave the club) using the realistic worst-case clearing rule instead of the optimistic "everyone eventually pays" baseline; **Only selected** restricts the totals/lists to just the selected members, a plain subset filter. Leaves the Kassenstand hero (whole-club real cash) and the Konten tab untouched. Pure logic in `lib/treasurySummary.ts` (`writeOffOutstandingDebt`)
