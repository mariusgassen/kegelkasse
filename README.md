...
- **History detail shows the same filterable protocol as the live evening** — tapping a closed evening's card expands its summary stats plus a full "⚠️ Strafen" timeline with player/game filter chips and "▶ started" / "🏁 finished · winner" game dividers (same logic `ProtocolPage` uses for the running evening, via `lib/protocolTimeline.ts`), an itemized "🍺 Getränke" round list, and the existing player roster, games and highlights sections — instead of only aggregated per-player totals and a drink count; no more reopening an evening just to see what happened, in what order, or filtered to one player/game
