...
- **Email/password login with rotating refresh-token sessions.** A login returns two credentials: a short-lived JWT access token (1 hour, `ACCESS_TOKEN_EXPIRE_MINUTES`) that every request carries, and an opaque refresh token (1 year of inactivity, `REFRESH_TOKEN_EXPIRE_DAYS`) that is stored server-side as a SHA-256 hash and is what actually keeps a member signed in between Kegelabende. When the access token expires the client transparently exchanges the refresh token for a new pair via `POST /auth/refresh` — the member never sees a login screen. Every refresh rotates the token and revokes its predecessor, so a copied token is only useful until the real client refreshes once; presenting an already-rotated token afterwards is treated as a replay and revokes that entire login lineage (a short grace window keeps two tabs refreshing at once from tripping it). Sessions are revocable: **Logout** ends just that device, a password change or reset ends every other one, and deactivating a member drops their sessions outright
...
- **Audio call-outs**: a club-wide toggle (Verein → Einstellungen → Audio-Ansagen, default on) for an optional synthesized sound picked per penalty type from a preview-able picker next to its icon. **30 presets** grouped by family — alarms (referee whistle, buzzer, air horn, alarm clock, phone ringtone), bells (bell, bicycle bell, fanfare, typewriter), money (cash register, coin drop), comedy (sad trombone, boing, slide whistle, crowd groan, applause, chicken, record scratch, chalk squeak), impacts (drum hit, punch, ball clack, breaking glass, splat), machines (jackhammer, lighter, laser) and water (water drop, trickle, toilet flush) — enough for every penalty type in a club roster to get its own. **Press and hold a penalty in the quick selection to hear its call-out without booking it**; a tap selects as before. All sounds are generated with the Web Audio API (no audio files) and share the same audio context as the celebration chimes. Gated by both the club switch and the personal 🎉 effects toggle. The synthesis goes well beyond plain oscillators — inharmonic partial stacks (bell, coin), FM (cash register, laser), band-passed noise off a shared buffer (applause, drum, breath), `playbackRate` automation (record scratch), parallel band-pass formants (crowd groan, chicken), wave-shaper grit (buzzer, air horn, jackhammer) and LFOs (whistle trill, trombone vibrato) — and every preset runs through one shared compressor + short convolution room so the catalog sits at a comparable loudness. `frontend/scripts/audit-sounds.mjs` renders each preset through an `OfflineAudioContext` in headless Chromium and checks level, duration, attack and spectral balance
- **Club configuration export/import**: `GET /club/config/export` bundles the club's *setup* — penalty types, game templates, teams, pins and base settings (venue, brand colors, guest penalty cap, no-show/pin fees, default evening time, throw-tracking and audio-callout toggles) — into a portable JSON file, downloadable from Verein → Einstellungen. `POST /club/config/import` seeds another club (e.g. a test/staging club) from that file. Deliberately excludes members, evenings, payments, the uploaded logo file, the PayPal.me handle, and the iCal/TV-scoreboard secret tokens, which must never be cloned into another club. Import is destructive by design — it replaces the target club's current penalty types/game templates/teams (soft-deleted, same as their normal delete endpoints) and pins (hard-replaced) — so the UI shows a confirmation sheet with the entry counts before calling the endpoint
- **Input hygiene**: request bodies are trimmed on the way in by a shared Pydantic base (`core/schemas.py::TrimmedModel`), so a stray leading/trailing space in a penalty type, pin, member, team or game-template name never reaches the database — where it would otherwise show up in reports, exports and the config bundle ("Pin fehlt " vs "Pin fehlt") and quietly break name matching. Passwords and secrets are exempt, since whitespace can legitimately be part of a credential. Migration 057 cleans up rows written before this
- **Penalty type ordering**: penalty types sort by amount ascending, then name — in the quick selection, in the admin list and in the config export. The old `sort_order` column was never editable in the UI (create hardcoded 99), so the grid came out in effectively arbitrary order while the tablet view already grouped by price; the column is dropped (migration 056) and bundles that still carry it import fine

### Evening management

- **Live mode (🔴)**: while an evening is running the app opens straight into an immersive live cockpit — the first sub-tab of the evening hub. A **scoreboard** (running game, whose turn it is, next player, last throw), a **stat row** (evening penalty total, beer/shot rounds, games finished), thumb-sized **quick actions** (Fine/Round → quick entry, Highlight → highlights tab, Games → games tab), and a chronological **event ticker** (penalties, drink rounds, highlights and 🎳 **all-nine** throws, newest first). Only milestone throws reach the ticker — an evening produces hundreds of throws, and the full stream would push everything else out of the capped feed within a single game, so the running throw state stays in the scoreboard; all-nine rows are hidden entirely for clubs with throw tracking switched off. Read-only over the active evening, kept fresh by the existing SSE + polling; the live tab disappears once the evening is closed. Pure derivation in `lib/liveEvening.ts` (`currentGameState`, `buildEventFeed`, `eveningTotals`), unit-tested.
- **📺 TV/beamer scoreboard**: a read-only full-screen display for a television or projector at the lane, opened at `/tv/<token>` with a secret-token link (same pattern as the iCal feed) so the screen never has to be logged in. Shows whose turn it is in the biggest type on the screen, the next player, the running game's throw history, the evening's headline numbers, and a standings panel that rotates every 12 s between **penalties today**, **drinks** and the **latest highlight photo** — skipping any panel with no data. King and Alle Neune take over the whole screen with the #62 confetti/sound celebration; moments that had already happened when the TV was switched on are never re-celebrated. Kiosk view, so dark-only on purpose, with the club's brand colour applied through the derived token set and viewport-relative type that reads from across the room. Admins copy, open and rotate the link under Verein → Einstellungen → 📺 TV-Scoreboard. Kept fresh by a slow poll plus a public SSE stream over the same in-memory event bus the authenticated evening stream uses
- Evenings are started from a scheduled entry (SchedulePage); no more ad-hoc creation — the old standalone start form (date/time/venue + "Let's go") is gone, `/evening` shows a hint pointing to the Schedule page instead when no evening is active
- **History detail can show the same filterable protocol as the live evening** — tapping a closed evening's card expands its summary stats, per-player penalty totals and a drink-count line by default (kept compact so a normal evening doesn't turn into a long scroll); a collapsed "📜 Full timeline"/"📜 All rounds" disclosure (shared `ExpandableCard`) opens on tap into the same detail the live evening's protocol has — player/game filter chips, "▶ started" / "🏁 finished · winner" game dividers (`lib/protocolTimeline.ts`, same logic `ProtocolPage` uses for the running evening) and an itemized round-by-round drink list — plus the existing player roster, games and highlights sections; no more reopening an evening just to see what happened, in what order, or filtered to one player/game
- Create evenings with date, optional venue override, and a free-text note
- Open/close toggle — closing an evening archives it to history; closing lets you set the evening's end timestamp (prefilled **in local wall-clock time** with the previously saved value, or now — it used to be prefilled in UTC), so you can backdate it if you forgot to close on time — the value is kept across reopen/re-close unless explicitly changed
...
- **📤 Shareable recap card**: any member can generate a shareable PNG summary of a closed evening (König, teuerste Strafe, durstigster Kegler, Kasse, Spiele beendet) from a prominent button right under the summary stats of the evening's expanded history detail (reachable under 📅 Termine, the club's only reachable history view — an earlier standalone `HistoryPage` route this button had originally shipped on was never wired into the router and has been removed) — rendered client-side on a canvas in the club's brand colors with the club logo, no server round-trip. Uses the Web Share API to hand the image straight to WhatsApp/etc. on mobile, falling back to a plain download on desktop. The "teuerste Strafe" headline excludes absence fees (booked for members who missed the evening entirely) — the pot total still includes them, but they're not treated as the evening's dramatic highlight; an evening with only absence bookings and no games/drinks shows no highlight row at all rather than a misleading one
...
- **Deactivate a member ahead of removal**: a lighter, reversible step before the guest-conversion above, for a member who has left but whose removal terms (entry fee/payout) aren't settled yet. "⏸️ Deaktivieren" locks their login and excludes them from future evenings — no more auto-booked absence penalties, no schedule RSVP prompts/reminders, and they no longer appear in the roster picker used to start an evening, backlog an unplanned one, or add a player mid-evening — while their treasury balance stays visible and untouched until settled. The deactivation date defaults to today but can be backdated to when the member actually left, so evenings in between are covered by the exclusion too. A deactivated member still shows up in the "known guests" picker at all three of those spots, so they can keep attending as a guest without being reactivated. "↩️ Reaktivieren" undoes it. A "🧹 Remove penalties since departure" action soft-deletes any penalties mistakenly logged for evenings after the deactivation date, in case some were booked before the member was actually marked as departed
...
- Dark/Light/System appearance toggle (Profile → Settings) — light mode is derived from the same per-club brand color (hue/saturation preserved, only lightness flips), so it also works for clubs with a custom background color; "System" follows the OS `prefers-color-scheme` live, without a reload, and the choice now also applies to the login screen and boot splash
- **Semantic color tokens with guaranteed contrast** — every color is derived at runtime from the club's brand colors plus the effective background, and every foreground is contrast-checked against the surface it lands on, so no combination of club branding and dark/light mode can produce unreadable text. Tokens come in pairs: `accent` fills, `accent-fg` writes (the same hue, darkened only as far as readability requires), `on-accent` sits on the fill. The same applies to the six team hues, status red/green, and medal tiers. A test asserts the whole contract in both modes across a spread of brand configurations
...
- **Collapsible side rail (≥1024px)** — a toggle at the top of the rail shrinks it from 224px to a 72px icon-only strip (labels stay in the DOM for screen readers, just visually hidden) and drops the 1040px content-width cap, handing that space to the page — useful for wide tables and future master-detail layouts. The choice persists across reloads; the mobile bottom bar is unaffected
...
- **One motion vocabulary instead of one-off keyframes** — shared duration/easing tokens (`fast` acknowledges a touch, `base` moves something in or out, `slow` rearranges a view, `ambient` loops quietly) exist as CSS custom properties *and* as JS constants, with a test pinning the two halves together. Built on top of it: **skeleton placeholders** shaped like the content that is coming (lists, accounts, charts) instead of a "Lade…" line, so nothing reflows when data lands — each one still announces itself as a loading state to a screen reader; **counting-up amounts** wherever money changes (cash on hand, the treasury flow rows, your own balance), which never animate on first paint; a **shared-element morph** growing a tapped row into its detail sheet via the View Transitions API, as progressive enhancement that falls back to the existing animation; and a **haptic vocabulary** (`selection` / `impact` / `success` / `warning` / `error`) wired centrally into every toast, so success and failure feel different with the tablet lying on the lane. `prefers-reduced-motion` now silences *every* animation (it previously covered three classes while four others kept running), and the 🎉 celebration switch additionally turns off the flourishes — haptics and count-ups included, skeletons excluded, since a loading state is not an effect
- **Date and time pickers speak local time** — every `date` / `datetime-local` field and its default now come from the user's own clock via shared helpers (`lib/datetime.ts`), instead of slicing `toISOString()`, which showed a UTC wall clock (a full offset off for a time picker, and the wrong day around midnight for a date picker). Submitting a time sends a timezone-aware instant so the backend, which reads a naive timestamp as UTC, stores what the user picked
- **Chart axes size themselves** — the left gutter of every y-axis is measured from its widest tick label instead of a fixed guess, and large values are abbreviated (`€1,2k` instead of `€1.234,56`), so labels no longer run out of the chart or collide with the rotated axis title. Applies to the evening cumulative chart, the scatter/dual-axis/Δ-bar charts in the Statistik-Labor and the treasury balance history; the dual-axis legend rows, which were spaced 8 units apart under a 12-unit font, no longer print on top of each other
- German and English translations, user-selectable
- **Type scale with a 12px floor** — nothing is smaller than 12px, and content a member actually reads (list subtitles, amounts, hints, notification bodies) is 14px. Hierarchy comes from weight, color and spacing rather than from shrinking text. Chart labels are sized per chart against their viewBox scale, so the effective on-screen size clears the floor too — the penalty donut's centre value used to render at 6 effective pixels
- Accessibility: WCAG-AA contrast for **all** text in both dark and light mode (verified by measuring the rendered DOM, not by inspection), keyboard-operable chart points/segments (StatsPage, TreasuryPage), focus-managed bottom sheets (focus moves in on open, restores to the trigger on close), and larger touch targets throughout
- **Icon language, enforced by a test** — Lucide icons for controls (always with a visible label or an `aria-label`), emoji for content (penalty types, badges, celebrations). Meaning is never carried by a `title` tooltip: there is no hover on a touch device, so a tooltip-only button is an unlabelled button. Every `title` on an interactive element is gone, every icon-only control has an accessible name, and explanations that used to hide in a tooltip on a *disabled* button are now visible text next to it. Dense icon clusters (schedule rows, the four club config lists, superadmin clubs) collapse into the shared **⋮ action menu**, which also puts a labelled, red "Löschen" between a mistap and a permanent delete. A lint test scans every `.tsx` and fails on a new `title` tooltip or a new nameless icon-only button, with an explicit exception list — so the rule survives the next feature instead of being re-learned
...
- **Shared UI primitives** (`components/ui/`) back the recurring patterns instead of each page re-rolling them: `<MemberRow>` (avatar + Kegelname + badge cluster + subtitle + trailing slot, tappable rows show a `›` chevron and rows without an action stay inert), `<Avatar>`, `<MemberBadges>`/`<MeBadge>`/`<PinBadges>`, `<StatTile>` (every metric grid), `<ExpandableCard>` (disclosure with the chevron and `aria-expanded` wired up), `<ActionSheet>` and `<ZoomableImage>`. The profile sheet no longer carries a second, private bottom-sheet implementation — it renders the shared `<Sheet>`, so it now also has a visible close button, Escape handling and a background scroll lock it previously lacked
- **Photos open fullscreen** — a posted comment/highlight/announcement photo (previously only ever shown at card-thumbnail size) now opens in a tap-to-dismiss fullscreen viewer (`<ZoomableImage>`), with the usual close button, Escape key and scrim-click, and focus returning to the thumbnail on close
