...
- **Donut-Diagramm**: Strafenverteilung pro Spieler (anwesend/abwesend). Spielerzeile in der Legende antippen, um zum Spieler-Detail zu wechseln
...
- **Strafe-pro-Getränk-Badge** (€ Strafe pro Getränk): Im Vergleichs-Modus zeigt sie den Abendsschnitt (z. B. „3,20 € pro Getränk"), im Fokus-Modus die persönliche Rate inklusive ±%-Vergleich zum Abendsschnitt (z. B. „+34 % mehr Strafe pro Getränk als der Abendsschnitt") und einer ausklappbaren Klartext-Erklärung. Unter dem Schnitt wird grün eingefärbt (billigere Runden), über dem Schnitt amber (jedes Getränk kostet mehr). Damit gibt es neben dem abstrakten Pearson *r* eine konkrete, vergleichbare Zahl pro Abend bzw. pro Person.
